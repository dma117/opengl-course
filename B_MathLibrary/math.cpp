...
float Vec4::GetScalarProductWith(const Vec4& other) const {
  ...
  //return this->x_ * other.x_ + this->y_ * other.y_ + this->z_ * other.z_ + this->w_ + other.w_;
  return x + y + z + w;
}
...
bool Vec4::operator==(const Vec4& other) const {
  return this->x_ == other.x_ && this->y_ == other.y_ && 
            this->z_ == other.z_ && this->w_ == other.w_;
}
...
Mat2 Mat2::GetIdentityMatrix() { 
  return { 1 };
}

Mat3::Mat3() {
  matrix_ = std::vector<Vec3>(3, 0);
}
...
Mat3 Mat3::GetIdentityMatrix() {
  return { 1 };
}

Mat4::Mat4() { 
  matrix_ = std::vector<Vec4>(4, 0);
}
...
Mat4::Mat4(float x1, float y1, float z1, float w1,
...
float Mat4::GetMinor(int row, int col) const { 
  ...
  return Mat3(values[0], values[1], values[2], 
              ...
              values[6], values[7], values[8]).GetDeterminant();
}
